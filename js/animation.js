...
//need it mostly when i rescale the web height;
let auxCanvasHeight;
...
$(window).on("resize", function(){
	...
	canvas.width = $(document).width();
	canvas.height = $(document).height() + $("table").height();
	canvasToDraw.width = canvas.width;
	canvasToDraw.height = canvas.height;
	...
	for(let i = 0; i < numberOfObjToCreate; i++){
		...
		if(i !== 0){
			for(let j = 0; j < objArray.length; j++){
				...
				//if so, reallocates the coords for the second obj (a smart idea would be to keep track of the coords that makes the second object spawn 
				//over the first one so that the algorithm doesn't pick the same ones again...but i m lazy :D)
